import streamlit as st
import requests

# הגדרות דף בסיסיות
st.set_page_config(page_title="מחשבון מטבע DK", page_icon="🇻🇳", layout="centered")

# --- חלק 1: עיצוב הלוגו DK והכפתור (CSS) ---
st.markdown(
    """
    <style>
    .main {
        direction: rtl;
    }
    .logo-wrapper {
        display: flex;
        justify-content: center;
        margin-bottom: 10px;
    }
    .logo-container {
        display: flex;
        flex-direction: column;
        align-items: center;
        justify-content: center;
        width: 90px;
        height: 90px;
        background-color: #2E7D32;
        border-radius: 50%;
        color: white;
        font-family: 'Arial Black', sans-serif;
        box-shadow: 0 4px 10px rgba(0,0,0,0.2);
        border: 2px solid #ffffff;
    }
    .logo-text {
        font-size: 34px;
        line-height: 1;
        font-weight: bold;
    }
    .logo-lines {
        width: 35px;
        height: 2px;
        background: white;
        margin-top: 5px;
        box-shadow: 0 4px 0 white; /* קו שני */
    }
    .pay-btn {
        background-color: #FFDD00;
        color: black !important;
        padding: 12px 24px;
        border-radius: 12px;
        text-decoration: none;
        font-weight: bold;
        display: inline-block;
        font-size: 18px;
        box-shadow: 0 4px 6px rgba(0,0,0,0.1);
    }
    .pay-btn:hover {
        background-color: #ffd000;
        transform: scale(1.05);
        transition: 0.2s;
    }
    </style>
    <div class="logo-wrapper">
        <div class="logo-container">
            <span class="logo-text">DK</span>
            <div class="logo-lines"></div>
        </div>
    </div>
    """,
    unsafe_allow_html=True
)

# --- חלק 2: פונקציה למשיכת שערים בזמן אמת ---
def get_live_rates():
    try:
        # API חינמי ויציב לשערי חליפין
        url = "https://open.er-api.com/v6/latest/USD"
        response = requests.get(url, timeout=5)
        data = response.json()
        
        usd_to_vnd = data['rates']['VND']
        usd_to_ils = data['rates']['ILS']
        
        # חישוב שקל לדונג
        ils_to_vnd = usd_to_vnd / usd_to_ils
        
        return ils_to_vnd, usd_to_vnd, True
    except:
        # שערים למקרה שאין אינטרנט (ערכי גיבוי)
        return 6800.0, 25000.0, False

# משיכת הנתונים
ils_to_vnd_rate, usd_to_vnd_rate, is_live = get_live_rates()

# --- חלק 3: כותרת וממשק המחשבון ---
st.markdown("<h1 style='text-align: center;'>מחשבון מטבע לטיול 🇻🇳</h1>", unsafe_allow_html=True)
st.markdown("<p style='text-align: center;'>הופכים מיליונים לשקלים ברגע</p>", unsafe_allow_html=True)

if is_live:
    st.caption("✅ שערים מעודכנים בזמן אמת מהרשת")
else:
    st.caption("⚠️ מצב אופליין: משתמש בשערי גיבוי")

st.divider()

# בחירת מטבע וסכום
option = st.selectbox("איזה מטבע ברצונך להמיר?", ["דונג (VND)", "שקל (ILS)", "דולר (USD)"])

# הגדרת סכום ברירת מחדל נוח לפי סוג המטבע
default_val = 100000.0 if option == "דונג (VND)" else 1.0
amount = st.number_input("הכניסי סכום:", min_value=0.0, value=default_val, step=1.0)

st.divider()

# לוגיקת החישוב והצגה
if option == "דונג (VND)":
    ils = amount / ils_to_vnd_rate
    usd = amount / usd_to_vnd_rate
    st.metric("בשקלים (ILS)", f"₪{ils:.2f}")
    st.metric("בדולר (USD)", f"${usd:.2f}")

elif option == "שקל (ILS)":
    vnd = amount * ils_to_vnd_rate
    usd = amount / (usd_to_vnd_rate / ils_to_vnd_rate)
    st.metric("בווייטנאמי (VND)", f"{vnd:,.0f} ₫")
    st.metric("בדולר (USD)", f"${usd:.2f}")

elif option == "דולר (USD)":
    vnd = amount * usd_to_vnd_rate
    ils = amount * (usd_to_vnd_rate / ils_to_vnd_rate)
    st.metric("בווייטנאמי (VND)", f"{vnd:,.0f} ₫")
    st.metric("בשקלים (ILS)", f"₪{ils:.2f}")

# הצגת שערי ההמרה לביקורת בתחתית
with st.expander("פרטי שערי החליפין"):
    st.write(f"1 ₪ = {ils_to_vnd_rate:,.2f} VND")
    st.write(f"1 $ = {usd_to_vnd_rate:,.2f} VND")

# --- חלק 4: כפתור פרגון (Buy Me a Coffee) ---
st.divider()
st.markdown("<div style='text-align: center;'>", unsafe_allow_html=True)
st.write("נהניתם מהמחשבון? עזר לכם בשוק, במונית או במסעדה? 🍜🚕")

# כאן תדביקי את הלינק שתקבלי מ-Buy Me a Coffee
pay_link = "https://www.buymeacoffee.com/yourname" 

st.markdown(f'<a href="{pay_link}" target="_blank" class="pay-btn">☕ פרגנו לי בקפה בווייטנאם (1$)</a>', unsafe_allow_html=True)
st.markdown("<p style='margin-top:15px; font-size: 12px;'>פותח באהבה על ידי מטיילת עבור מטיילים</p>", unsafe_allow_html=True)
st.markdown("</div>", unsafe_allow_html=True)
